...
const validationSchema = Yup.object()
  .shape({
    ...
    nameField: Yup.string().max(50, "Deck name must be 50 characters or less"),
    linkField: Yup.string().matches(
      /^(https?):\/\/[^\s$.?#].[^\s]*$/,
      "Invalid URL format"
    ),
    tagsField: Yup.string().max(100),
    ...
    eitherTextOrFileOrUrlError: Yup.string().test(
      ...
      "You must provide exactly one input: a file, a link, or some text",
      function (value) {
    ...
    eitherNameOrExistingDeckError: Yup.string().test(
      ...
      "Either provide a name for a new deck or choose an existing deck",
      function (value) {
  ...
  .test(
    "either-text-or-file-or-url",
    "Provide exactly one input: a file, a link, or some text to process",
    (value) => {
  ...
  .test(
    ...
    "Either provide a name for a new deck or choose an existing deck",
    (value) => {
      const inputs = [value.nameField, value.existingDeckField];
      const filledInputs = inputs.filter((input) => {
        return typeof input === "string" ? input.trim() !== "" : Boolean(input);
      });
      return filledInputs.length === 1;
