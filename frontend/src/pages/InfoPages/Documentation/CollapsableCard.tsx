...
import type { GuideType } from "./documentation";
import { GuideContent } from "./GuideContent";

interface CollapsableCardProps {
  guideSection: GuideType;
}
const CollapsableCard: React.FC<CollapsableCardProps> = ({ guideSection }) => {
  return (
    <>
      <Disclosure>
        {({ open }) => (
          <>
            <Disclosure.Button className="flex w-full  items-center justify-between rounded-lg px-4  py-2 text-left text-xl text-white   focus:outline-none  focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-[75%] bg-mariana-blue hover:bg-mariana-blue-100">
              <div className="flex items-center gap-2">
                {" "}
                <img src={guideSection.icon} />
                <span>{guideSection.mainTitle}</span>
              </div>
              <ChevronUpIcon
            ...
            >
              <Disclosure.Panel className="px-4 pb-2 pt-4 text-sm text-white">
                <p>{guideSection.useSummary} </p>
                <GuideContent guideSection={guideSection} />
              </Disclosure.Panel>
            </Transition>
          </>
        )}
