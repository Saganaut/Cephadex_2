...
const DiscordLoginButton = (): React.JSX.Element => {
  if (
    VITE_APP_BASE_URL.length === 0 ||
    DISCORD_REDIRECT_URI.length === 0 ||
    DISCORD_CLIENT_ID.length === 0
  ) {
    console.error("Discord OAuth configuration is missing.");
    return <div>Error: Discord OAuth configuration is missing.</div>;
  }
  const fullDiscordRedirectUri = `${VITE_APP_BASE_URL}${DISCORD_REDIRECT_URI}`;
