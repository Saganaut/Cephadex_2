...
class Game(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'time-created': self.time_created.strftime("%Y-%m-%d %H:%M:%S") if self.time_created else None,
            'start-time': self.start_time.strftime("%Y-%m-%d %H:%M:%S") if self.start_time else None,
