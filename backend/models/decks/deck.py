...
class Deck(db.Model):
    ...
    cards = db.relationship(
        "Card", secondary=cards, backref="decks_backref", lazy="select"
    )
    deck_files = db.relationship(
        "DeckFiles", secondary=source_files, backref="decks", lazy="select"
    )
        ...
        for card in self.cards:
            if card.times_correct is not None and card.times_asked is not None:
                try:
                    ...
                    incorrect = (
                        incorrect + int(card.times_asked) - int(card.times_correct)
                    )
                except ValueError:
    ...
    def to_dict(self) -> json:
        return {
            ...
            "qty-cards": self.qty_cards,
            "qty-cards-due": self.qty_cards_due(),
            "time-created": self.time_created.isoformat()
            if self.time_created
            else None,
            "time-updated": self.time_updated.isoformat()
            ...
            "times-accessed": self.times_accessed,
            "access-date": self.access_date.isoformat() if self.access_date else None,
            ...
            "sharer": self.sharer,
            "source": self.source,
            "share-date": self.share_date.isoformat() if self.share_date else None,
    ...
    def copy_deck(self, new_deck_name):
        new_deck = Deck(
            name=self.name, description=self.description, user_id=self.user_id
        )
